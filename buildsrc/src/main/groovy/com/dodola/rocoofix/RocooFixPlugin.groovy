...
class RocooFixPlugin implements Plugin<Project> {
    ...
    private void applyTask(Project project, DomainObjectCollection<BaseVariant> variants) {
        ...
        project.afterEvaluate {
            ...
            def excludePackage = rocooConfig.excludePackage
...
//                    SigningConfig signingConfig = variant.getBuildType().getSigningConfig()
//                    if(signingConfig != null){
//                        println("==========" + signingConfig.keyPassword);
//                        println("==========" + signingConfig.storePassword);
//                        println("==========" + signingConfig.keyAlias);
//                    }
                    ...
                    project.task(rocooPatchTaskName) << {
                        if (patchDir) {
                            ...
                            File patchJar = new File(patchDir.getParent(), PATCH_NAME)
                            RocooUtils.makeDex(project, patchDir, patchJar)

                            SigningConfig signingConfig = variant.getBuildType().getSigningConfig()
                            if(signingConfig != null){
                                RocooUtils.signJar(patchJar, signingConfig.storeFile, signingConfig.keyPassword,
                                signingConfig.storePassword, signingConfig.keyAlias)
                            }
                        }
                    }
                        ...
                        project.task(rocooJarBeforePreDex) << {
                            ...
                            inputFiles.each { inputFile ->
                                ...
                                if (NuwaProcessor.shouldProcessPreDexJar(path)) {
                                    NuwaProcessor.processJar(hashFile, inputFile, patchDir, hashMap, includePackage, excludeClass, excludePackage)
                                }
                            }
                        }
                                ...
                                if (path.endsWith(".class") && !path.contains("/R\$") && !path.endsWith("/R.class") && !path.endsWith("/BuildConfig.class")) {
                                    if (NuwaSetUtils.isIncluded(path, includePackage)) {
                                        if (!NuwaSetUtils.isExcluded(path, excludeClass)
                                                && !NuwaSetUtils.isExcludedInPackage(path, excludePackage)) {
                                            ...
                                            if (classPath.endsWith(".class") && !classPath.contains("/R\$") && !classPath.endsWith("/R.class") && !classPath.endsWith("/BuildConfig.class")) {
                                                if (NuwaSetUtils.isIncluded(classPath, includePackage)) {
                                                    if (!NuwaSetUtils.isExcluded(classPath, excludeClass)
                                                            && !NuwaSetUtils.isExcludedInPackage(classPath, excludePackage)) {
